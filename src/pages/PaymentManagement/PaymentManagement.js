...
export default function PaymentManagement() {
    ...
    const loadAllPayments = async (currentPage, status, orderCode = '', customerName = '', email = '', trackingId = '', orderDate = '') => {
        // clearFiltrationFields();
        popUploader(dispatch, true);

        try {
            const resp = await searchFiltration(currentPage, status, orderCode, customerName, email, trackingId, orderDate);
    ...
    const debouncedLoadAllPayments = debounce((currentPage, status, orderCode, customerName, email, trackingId, orderDate) => {
        loadAllPayments(currentPage, status, orderCode, customerName, email, trackingId, orderDate);
    }, 300);

    const debounceHandleSearchPaymentFiltration = (currentPage = 1, status, orderCode, customerName, email, trackingId, orderDate) => {
        debouncedLoadAllPayments(currentPage, status, orderCode, customerName, email, trackingId, orderDate);
    };
