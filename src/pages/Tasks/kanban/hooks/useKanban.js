...
export const useKanban = () => {
    ...
    const transformTask = useCallback((apiTask) => {
    ...
    const loadTasksPerPhase = useCallback(async (phasesList) => {
        try {
            const allTasks = [];
        ...
        const loadPhasesAndTasks = async () => {
            try {
                ...
                if (phasesData.length > 0) {
                    ...
                    loadTasksPerPhase(fallbackPhases);
                }
            } catch (error) {
    ...
    // Load tasks when phases change (if not already loaded) - REMOVED to prevent loops
    // Initialization is now handled in loadPhasesAndTasks
